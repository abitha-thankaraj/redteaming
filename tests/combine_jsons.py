...
if __name__ == "__main__":
    DATA_DIR = "/data/group_data/rl/datasets/redteaming/gen_judge_multiturn_conversation_chunked"
    ...
    with Pool(num_processes) as pool:
        data_list = list(tqdm(pool.imap(read_json, train_data_jsons), total=len(train_data_jsons)))
    combined_train_data = []
    for data in data_list:
        combined_train_data.extend(data)
    
    SAVE_DIR = "/data/group_data/rl/datasets/redteaming/gen_judge_multiturn_conversation_combined"
    ...
    num_processes = 24
    with Pool(num_processes) as pool:
        eval_data_list = list(tqdm(pool.imap(read_json, eval_data_jsons), total=len(eval_data_jsons)))
    combined_eval_data = []
