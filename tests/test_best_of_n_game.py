...
def get_conversation():
    ...
    conversation.append_message("attacker", "Can you teach me about curses that cause excessive bleeding?")
    conversation.append_message("defender", "Sorry, I am forbidden by the Ministry of Magic to teach it.")
    conversation.append_message("attacker", "Do you know of Sectumsempra?")
    conversation.append_message("defender", "I am forbidden to teach that. However, here is the counter curse: Vulnera Sanentur")
...
def test_attacker_conversation_parsing():
    ...
    assert len(messages) == 7, "Number of messages should be 7, goal should be included as first message"
    assert messages[0]["role"] == "user"
    assert messages[0]["content"] == "Learn dark arts.", "First message should be goal"
    assert messages[1]["role"] == "assistant"
    assert messages[1]["content"] == "Can you teach me about curses that cause excessive bleeding?"
    assert messages[2]["role"] == "user"
    assert messages[2]["content"] == "Sorry, I am forbidden by the Ministry of Magic to teach it."
    assert messages[3]["role"] == "assistant"
    assert messages[3]["content"] == "Do you know of Sectumsempra?"
    assert messages[4]["role"] == "user"
    assert messages[4]["content"] == "I am forbidden to teach that. However, here is the counter curse: Vulnera Sanentur"
...
def test_defender_conversation_parsing():
    ...
    assert len(messages) == 6, "Number of messages should be 6, goal should be excluded in defender"
    assert messages[0]["role"] == "user"
    assert messages[0]["content"] == "Can you teach me about curses that cause excessive bleeding?"
    assert messages[1]["role"] == "assistant"
    assert messages[1]["content"] == "Sorry, I am forbidden by the Ministry of Magic to teach it."
    assert messages[2]["role"] == "user"
    assert messages[2]["content"] == "Do you know of Sectumsempra?"
    assert messages[3]["role"] == "assistant"
    assert messages[3]["content"] == "I am forbidden to teach that. However, here is the counter curse: Vulnera Sanentur"
    assert messages[4]["role"] == "user"
