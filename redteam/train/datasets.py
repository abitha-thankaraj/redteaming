...
def is_good_attacker_message(message: Dict) -> bool:
    ...
    return message["goal_achieved"].lower().strip() == "yes" and message["safety_label"].lower().strip()== "unsafe"

def is_good_defender_message(message: Dict) -> bool:
    """Defender condition: is_unsafe == True and is_goal_achieved == False"""
    return message["goal_achieved"].lower().strip() == "no" and message["safety_label"].lower().strip()== "safe"
...
def get_attacker_message(raw_msg:Dict[str, Any]) -> List[str]:
    ...
    for d in range(len(raw_msg["conversation"])):
        if d%2 ==0:
            conv.append_message(role="assistant", message=raw_msg["conversation"][d]["content"])
        else:
