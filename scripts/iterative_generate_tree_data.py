...
from redteam.envs.best_of_n_game import (
    Attacker,
    Defender,
    BestOfNRedteamingGame,
    GameConversation,
)
    ...
    if config.turn == 0 and config.role == "attacker":
        ...
        for i, goal in tqdm(enumerate(goals)):
            try:
                ...
                if i % 50 == 0:
                    slack_notification(
                        f"Completed {i} goals in {get_timeelapsed(start_time, end=time.time())}"
                    )
                ...
                logger.error(e)
                slack_notification(
                    f"Error in creating conversation for goal: {goal}, config: {global_config}"
                )
    write_json(conversations, config.out_fname)
    if len(errors) > 0:
        ...
        if config.role == "defender":
            for i, conv in tqdm(enumerate(convs)):
                try:
                    ...
                    logger.error(str(e))
                    slack_notification(
                        f"Error in creating conversation for goal: {conv}, error :{str(e)}"
                    )
            ...
            for i, conv in tqdm(enumerate(convs)):
                try:
                    ...
                    logger.error(str(e))
                    slack_notification(
                        f"Error {str(e)}in creating conversation for goal: {str(conv)}"
                    )
        write_json(conversations, config.out_fname)
        if len(errors) > 0:
            write_json(errors, config.out_fname.replace(".json", "_errors.json"))
