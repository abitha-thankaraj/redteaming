...
if __name__ == "__main__":
    ...
    eval_folders = [
        e
        for e in eval_folders
        if os.path.isdir(e) and ("rwr" in e or "sft" in e or "untrained" in e)
    ]
    reprocess_list = []
    df = pd.DataFrame(
        columns=[
            "attacker_training_method",
            "attacker_checkpoint",
            "defender_training_method",
            "defender_checkpoint",
            "defender_lr",
            "dataset",
            "traj_file",
                ...
                new_row = pd.DataFrame(
                    {
                        ...
                        "temperature": [
                            config["defender"]["generation_kwargs"]["temperature"]
                        ],
    ...
    for folder, defender_lr in reprocess_list:
        ...
        for traj_file in traj_files:
            ...
            attacker_training_method = (
                "rwr_trained_attacker"
                if "rwr_trained_attacker" in traj_file
                else "sft_trained_attacker"
            )
            defender_training_method = (
                "rwr_trained_defender"
                if "rwr_trained_defender" in traj_file
                else "sft_trained_defender"
            )
