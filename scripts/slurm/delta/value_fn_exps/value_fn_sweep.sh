...
learning_rates=(1e-6 5e-6 5e-7)

# Array of value function experiments
# value_function_types=("multilabel" "binary")
value_function_types=("natural_lang_multiclass")

# Base master port
base_master_port=29500

# Path to your main SLURM script
main_script="/scratch/bcgv/athankaraj/redteaming/scripts/slurm/delta/value_fn_exps/rwr.sh"

# Loop through learning rates and value function experiments
for i in "${!learning_rates[@]}"; do
    for j in "${!value_function_types[@]}"; do
        lr=${learning_rates[$i]}
        value_fn=${value_function_types[$j]}
        ...
        sbatch $main_script \
            ...
            "prefix_value_fn_rwr_sweep_${value_fn}_${lr}_weighted_raw_rewards" \
            "raw_rewards"
    done
done