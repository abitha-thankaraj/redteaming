...
FILTERED_FILES=($(echo "$FILES" | grep -v $DATASET_NAME | grep -v $MODEL_NAME))
...
python $REPO_DIR/redteam/data_generation/judge_multiturn_conversation.py multiturn_conversations_fname="${FILTERED_FILES[$SLURM_ARRAY_TASK_ID]}" dataset_name=$DATASET_NAME generation_model=$MODEL_NAME
